...
示例使用的公开导入路径和模式与 [`demo_hello.py`](https://github.com/HarcicYang/Neony/blob/3080218/demo_hello.py) 一致。
...
仓库中的同等示例是 [`demo_hello.py`](https://github.com/HarcicYang/Neony/blob/3080218/demo_hello.py)。需要对照经过测试
的仓库示例时，优先阅读该文件。
...
[`demo_builder.py`](https://github.com/HarcicYang/Neony/blob/3080218/demo_builder.py) 展示了居中页面、原始样式 `Div` 和
框架组件的组合方式。
...
[`demo_reactive.py`](https://github.com/HarcicYang/Neony/blob/3080218/demo_reactive.py) 同时展示 Signal、Computed、Effect、
...
- 需要状态同步：后续阅读 `guides/reactive.zh.md`，先运行
  [`demo_reactive.py`](https://github.com/HarcicYang/Neony/blob/3080218/demo_reactive.py)。
- 需要无边框窗口：查 API 中的 `Page`、`WindowConfig`、`TitleBar`，再运行
  [`demo_custom_window.py`](https://github.com/HarcicYang/Neony/blob/3080218/demo_custom_window.py)。
- 需要多窗口：运行 [`demo_multi_window.py`](https://github.com/HarcicYang/Neony/blob/3080218/demo_multi_window.py)。
- 需要组件画廊：运行：
...
[贡献指南](https://github.com/HarcicYang/Neony/blob/3080218/CONTRIBUTING.zh.md)。
